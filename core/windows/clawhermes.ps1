param(
    ...
    [ValidateSet("env-json", "setup", "runtimes", "payloads", "payload-export", "wsl", "prepare-wsl", "wsl-rootfs-guide", "wsl-import-plan", "wsl-import", "wsl-export", "wsl-unregister-plan", "wsl-unregister", "wsl-workflow", "install-runtime", "init-env", "service-env", "adapters", "sources", "probe-sources", "checkout-source", "setup-adapter", "verify-adapter", "mark-adapter-ready", "logs", "backup", "restore-plan", "restore", "start", "start-adapter", "status", "stop")]
    [string]$Action = "setup",
