import { adapterSetupPlan, appSourcePlan, checkoutAppSource, createBackup, getRoot, getStatus, initializeEnvFiles, installRuntimeFromArchive, markAdapterReady, portableEnv, prepareWsl, probeAppSources, readLogTail, runAdapterSetup, runtimePreparationPlan, serviceEnvironmentDiagnostic, setupDiagnostics, startSingleAdapter, startSkeleton, stopSkeleton, verifyAdapter, writeStatusSnapshot, wslDiagnostics, wslImport, wslImportPlan, wslRootfsGuide, wslUnregisterPlan, wslWorkflowPlan } from "./core";
...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  let profile: BackupProfile = "data-only";
...
function parseBackupProfile(value: string): BackupProfile {
  if (value === "data-only" || value === "full") return value;
...
async function main(): Promise<void> {
  ...
  switch (action) {
    ...
    case "wsl-import": {
      const result = wslImport(root, { distro, confirmImport });
      if (json) {
        printJson(result);
      } else {
    ...
    case "wsl-unregister-plan": {
      const result = wslUnregisterPlan(root, { distro });
      if (json) {
        printJson(result);
      } else {
    ...
    case "checkout-source": {
      const result = checkoutAppSource(root, positional[0], { dryRun, confirm: confirmCheckout });
      if (json) {
        printJson(result);
      } else {
    ...
    case "backup": {
      const result = createBackup(root, { profile, includeLogs, dryRun });
      if (json) {
        printJson(result);
      } else {
        console.log(result.message);
    ...
    case "start-adapter": {
      const result = startSingleAdapter(root, positional[0], { dryRun, confirm: confirmStart });
      if (json) {
        printJson(result);
      } else {
        console.log(result.message);
