...
type WorkflowPhase = {
  id: string;
  title: string;
  status: "ready" | "needed" | "manual" | "blocked";
  command: string;
...
export function wslWorkflowPlan(usbRoot: string, serviceId: string | undefined) {
  ...
  if (!serviceId) throw new Error("Service id is required. Example: wsl-workflow hermes-agent --json");
  const adapter = loadAdapters(root).find((item) => item.id === serviceId);
  if (!adapter) throw new Error(`Unknown adapter: ${serviceId}`);
  if (adapter.runtime?.kind !== "wsl2") throw new Error(`Adapter ${serviceId} is not a WSL2 adapter.`);
  ...
  const importPlan = wslImportPlan(root, { distro: sourceDistro });
  const unregisterPlan = wslUnregisterPlan(root, { distro: sourceDistro });
