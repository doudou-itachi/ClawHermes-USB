...
export function setupWizard(usbRoot: string) {
  const root = getRoot(usbRoot);
  const setup = setupDiagnostics(root);
  const payloads = payloadInventory(root);
  const adapters = loadAdapters(root).filter((adapter) => adapter.enabled);
  const orderedAdapters = serviceOrder(root, "start").filter((adapter) => adapter.enabled);
...
function command(label: string, command: string, mutatesState: boolean, requiresConfirmation: boolean): WizardCommand {
