...
- Portal: `17000`
- OpenClaw gateway/control/webchat: `18789` in the verified WSL2 adapter path.
...
- Hermes Agent uses the WSL2 adapter path for Windows, runs in the project-managed `ClawHermes-Ubuntu` distro, and exposes health at `http://127.0.0.1:8642/health`.
- OpenClaw uses the WSL2 adapter path for Windows, runs in the project-managed `ClawHermes-Ubuntu` distro, stores state under `data/openclaw`, and exposes health at `http://127.0.0.1:18789/healthz`.
- OpenClaw source-checkout startup is `node openclaw.mjs gateway --port ${OPENCLAW_GATEWAY_PORT} --verbose --allow-unconfigured` after setup verifies Node.js 24 and pnpm 10.33.2.
- WSL2 rootfs archives, WSL backups, upstream app checkouts, and dependency folders are local ignored payloads. They are not source files and should be regenerated or distributed through a separate packaging flow.

Remaining open questions:

- What packaging flow should create or refresh the ignored WSL rootfs/app payload bundle for end users?
