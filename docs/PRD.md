...
4. Checks port availability and assigns replacement localhost ports when defaults are occupied.
5. Starts OpenClaw.
6. Starts Hermes Agent gateway.
7. Starts Hermes Web UI.
8. Starts the local portal.
9. Opens the assigned local portal URL, normally `http://127.0.0.1:17000/`.
...
- The launcher must automatically remap occupied default localhost ports to free replacements before starting services.
- Runtime assignments must be written to `data/tmp/ports.json`.
- Service env variables, health URLs, portal URLs, and templated start commands must use runtime-assigned ports.

Initial reserved ports:

- Portal: `17000`
- OpenClaw gateway/control/webchat: `18789` in the verified Windows-native adapter path.
...
Host machines may already use required ports. Mitigation: detect conflicts before startup, remap occupied localhost ports, and show the actual assigned URLs in status and portal metadata.
...
- Runtime port remapping is enabled for occupied default localhost service and portal ports; assignments are stored under `data/tmp/ports.json`.
- Hermes Agent defaults to a Windows-native verified adapter using a project-local Python virtual environment and health at `http://127.0.0.1:8642/health`.
- OpenClaw defaults to a Windows-native verified adapter using Node.js, pnpm, state under `data/openclaw`, and health at `http://127.0.0.1:18789/healthz`.
- OpenClaw source-checkout startup is `node openclaw.mjs gateway run --port ${OPENCLAW_GATEWAY_PORT} --verbose --allow-unconfigured` after setup prepares pnpm dependencies.
- The PyQt control panel is a thin desktop client that starts or reuses a localhost Node control service and communicates through JSON APIs.
- Local Windows verification on 2026-05-06 used OpenClaw `82c4fd8f56751faa03470e32f4763b7245c69b71` and Hermes Agent `f27fcb6a82b8487174ca941c15e7a5887371eede`.
- Optional WSL2 rootfs archives, WSL backups, upstream app checkouts, and dependency folders are local ignored payloads. They are not source files and should be regenerated or distributed through a separate packaging flow.

Remaining open questions:

- What packaging flow should create or refresh ignored upstream app payloads and optional WSL rootfs bundles for end users?
- Should release builds pin the verified upstream revisions exactly, or refresh to upstream `main` and re-run adapter verification during packaging?
