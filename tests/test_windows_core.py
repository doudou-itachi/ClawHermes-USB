...
class WindowsCoreTests(unittest.TestCase):
    ...
    def test_pyqt_control_panel_bootstraps_control_server_and_polls_status(self):
        ...
        self.assertIn("cleanup_before_exit", script)
        self.assertIn("wait_for_control_server_shutdown", script)
        self.assertIn("kill_control_server_from_metadata", script)
...
"""
    ...
    def test_macos_launchers_prepare_runtime_and_prefer_electrobun_app(self):
        ...
        self.assertIn("data/logs/macos-launcher.log", start)
        ...
        self.assertIn("ClawHermes-Control-Mac.app.tar.gz", start)
        self.assertIn("APP_BUNDLED_PATH=", start)
        self.assertIn("APP_CACHE_ROOT=", start)
        self.assertIn("Library/Application Support/ClawHermes-USB/macos-app", start)
        self.assertIn("APP_EXTRACT_DIR=", start)
        self.assertIn("APP_EXECUTABLE=", start)
        self.assertIn("stageElectrobunAppBundle", start)
        self.assertIn("Staging Electrobun UI app locally from archive", start)
        self.assertIn("Staging Electrobun UI app locally from bundle", start)
        self.assertIn("Electrobun UI app archive did not contain a .app bundle", start)
        self.assertIn("verifyElectrobunAppBundle", start)
        self.assertIn("Electrobun UI app bundle verified", start)
        self.assertIn("Electrobun UI app bundle is incomplete", start)
        ...
        self.assertIn("electrobunPidsForCurrentBundle", start)
        self.assertIn("No Electrobun process for current bundle was found", start)
        ...
        self.assertIn("timeout", start)
        self.assertIn('tell application id "dev.clawhermes.control" to activate', start)
        self.assertIn("current bundle", start)
        self.assertIn("Electrobun UI did not stay running", start)
        self.assertIn("Electrobun UI did not expose a visible window", start)
        self.assertIn("APP_OPENED=0", start)
        ...
        self.assertIn("data/logs/macos-launcher.log", stop)
        self.assertIn("Stopping ClawHermes from", stop)
    ...
    def test_electrobun_control_shell_scaffold_matches_vh_claw_style(self):
        ...
        electrobun_config = (shell_root / "electrobun.config.ts").read_text(encoding="utf-8")
        build_script = (shell_root / "build.ps1").read_text(encoding="utf-8")
        portable_wrapper = (shell_root / "portable-wrapper" / "ClawHermesControlLauncher.cs").read_text(encoding="utf-8")
        ...
        self.assertIn("vue", package_json["devDependencies"])
        self.assertIn("vite build", package_json["scripts"]["build:web"])
        self.assertIn("node ./node_modules/electrobun/bin/electrobun.cjs", package_json["scripts"]["build:win"])
        self.assertIn("HTTP_PROXY", build_script)
        self.assertIn("127.0.0.1:7897", build_script)
        self.assertIn("node .\\node_modules\\electrobun\\bin\\electrobun.cjs build", build_script)
        self.assertIn("ClawHermes-Control-Electrobun.exe", build_script)
        self.assertIn("portable-wrapper\\ClawHermesControlLauncher.cs", build_script)
        self.assertIn("views/mainview/index.html", electrobun_config)
        self.assertIn("exitOnLastWindowClosed: false", electrobun_config)
        self.assertIn("CLAWHERMES_USB_ROOT", portable_wrapper)
        self.assertIn("ClawHermes-Control-Electrobun-Setup.exe", portable_wrapper)
        self.assertIn("dev.clawhermes.control", portable_wrapper)
        self.assertIn("NeedsInstall", portable_wrapper)
        self.assertIn("PrepareForInstall", portable_wrapper)
        self.assertIn("StopInstalledApp", portable_wrapper)
        self.assertIn("DeleteDirectoryWithRetry", portable_wrapper)
        self.assertIn("ClawHermes-Control-Electrobun-Setup.metadata.json", portable_wrapper)
        self.assertIn("clawhermes-setup-hash.txt", portable_wrapper)
        self.assertIn("BrowserWindow", bun_entry)
        self.assertIn("BrowserView.defineRPC", bun_entry)
        self.assertIn("control-server", bun_entry)
        self.assertIn("data\", \"logs\", \"electrobun-control.log", bun_entry)
        self.assertIn("writeElectrobunLog", bun_entry)
        self.assertIn("let controlUrl", bun_entry)
        self.assertIn("async function getControlUrl", bun_entry)
        self.assertIn("void getControlUrl()", bun_entry)
        self.assertLess(bun_entry.index("mainWindow = new BrowserWindow"), bun_entry.index("void getControlUrl()"))
        self.assertIn("/api/services/start", bun_entry)
        self.assertIn("/api/channels/weixin/login", bun_entry)
        self.assertIn("/api/channels/weixin/stop", bun_entry)
        self.assertIn("getWeixinChannelStatus", bun_entry)
        ...
        self.assertIn("maximizeWindow", vue_app)
        ...
        self.assertIn('id: "channels"', vue_app)
        ...
        self.assertIn("workspaceRef", vue_app)
        self.assertIn("scrollServiceSection", vue_app)
        self.assertNotIn('href="#service-', vue_app)
        self.assertIn("DTClaw 即插即用", vue_app)
        self.assertIn("赠送100万Tokens", vue_app)
        self.assertIn("联系客服获取更多支持", vue_app)
        self.assertIn("进入官网即可立即升级企业级数字员工", vue_app)
        self.assertIn("注：此图片为参考图收到的产品批次不同款式不同", vue_app)
        self.assertIn("产品优势", vue_app)
        self.assertIn("应用场景", vue_app)
        self.assertIn("增值服务", vue_app)
        self.assertIn("基础标配服务", vue_app)
        self.assertIn("9大核心增值服务", vue_app)
        self.assertIn("全行业垂直AI解决方案", vue_app)
        self.assertIn("serviceValueServices", vue_app)
        self.assertIn("serviceScenarios", vue_app)
        self.assertNotIn(">联系我们<", vue_app)
        self.assertNotIn(">DT 官网<", vue_app)
        ...
        self.assertIn("selectProvider", vue_app)
        self.assertIn("selectedProviderId", vue_app)
        self.assertIn("serviceTitle", vue_app)
        self.assertIn("Hermes Web UI", vue_app)
        self.assertIn("dtclawLogo", vue_app)
        self.assertIn("DTclaw", vue_app)
        self.assertIn("融云API", vue_app)
        self.assertIn("https://models.mixedcloud.cn/v1", vue_app)
        self.assertIn("https://models.mixedcloud.cn", vue_app)
        self.assertIn("自定义", vue_app)
        self.assertNotIn("https://api.siliconflow.cn/v1", vue_app)
        self.assertNotIn("DeepSeek", vue_app)
        self.assertNotIn("MiniMax", vue_app)
        self.assertNotIn("Kimi", vue_app)
        self.assertNotIn("Claude", vue_app)
        self.assertNotIn("Groq", vue_app)
        self.assertIn("openClawHub", vue_app)
        self.assertIn("https://cn.clawhub-mirror.com/", vue_app)
        self.assertIn("6万+ Agent Skill", vue_app)
        self.assertIn("ClawHub镜像站", vue_app)
        self.assertIn("IconGlyph", vue_app)
        self.assertIn("serviceIcon", vue_app)
        self.assertIn("linear-gradient(135deg, #111827", styles)
        self.assertIn(".sidebar", styles)
        self.assertIn(".provider-card", styles)
        self.assertIn(".window-bar", styles)
        self.assertIn(".nav-icon", styles)
        self.assertIn(".service-icon", styles)
        self.assertIn(".action-button", styles)
        self.assertIn(".action-icon", styles)
        self.assertIn(".service-name", styles)
        self.assertIn(".service-subtitle", styles)
        ...
        self.assertIn(".service-hero-note", styles)
        self.assertIn("flex-direction: column", styles)
        self.assertIn(".service-info-grid", styles)
        self.assertIn(".service-landing-section", styles)
        self.assertIn(".service-scenario-grid", styles)
        self.assertIn(".service-value-grid", styles)
        self.assertIn(".service-solution-band", styles)
        self.assertIn(".skill-market-card", styles)
        self.assertIn(".skill-market-actions", styles)
        self.assertIn("height: 100vh", styles)
        self.assertIn("overflow: hidden", styles)
        self.assertIn(".service-nav-links button", styles)
    ...
    def test_openclaw_channel_routes_match_vh_claw_depth(self):
        ...
        self.assertIn("/api/channels/weixin/login", control_server)
        self.assertIn("/api/channels/weixin/stop", control_server)
        self.assertIn("@tencent-weixin", channels)
        self.assertIn("openclaw-weixin", channels)
        self.assertIn("channels", channels)
        self.assertIn("login", channels)
        self.assertIn("plugins", channels)
    ...
    def test_usb_release_script_prunes_development_app_payloads(self):
        ...
        try:
            ...
            (source_root / "runtimes" / "wsl" / "ubuntu-rootfs.tar").write_text("rootfs\n", encoding="utf-8")
    ...
    def test_usb_release_script_emits_macos_entrypoints_and_runtime_archives(self):
        ...
        try:
            ...
            self.assertTrue((output_root / "ClawHermes-Control-Mac.app.tar.gz").exists())
            ...
            self.assertIn("ClawHermes-Control-Mac.app.tar.gz", manifest["platformPayloads"]["macos"])
            self.assertIn("ClawHermes-Control-Mac.app.tar.gz", json.dumps(manifest["copiedPaths"]))
